...
describe("SupplyRows", () => {
  ...
  it("saving a link records it and shows the retailer", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Edit Furnace filter" }))
    fireEvent.change(screen.getByLabelText("Link for Furnace filter"), { target: { value: "https://homedepot.com/p/1" } })
    fireEvent.click(screen.getByRole("button", { name: "Save" }))
    await waitFor(() => expect(updateTaskSupply).toHaveBeenCalledWith("h1", "t1", 0, { name: "Furnace filter", url: "https://homedepot.com/p/1", size: null }))
  ...
  it("Edit lets you replace the parsed name with your own specifics — one save carries name, link and size", async () => {
    const vague = { ...filter, name: "Field-supplied return air filter (e.g. 16×25 or 20×25)", url: null, size: null }
    updateTaskSupply.mockResolvedValue({ data: { ...vague, name: "16×25×1 MERV 8 filter", size: "16x25x1" }, error: null })
    render(<SupplyRows homeId="h1" taskTemplateId="t1" supplies={[vague]} nextInstanceId="i1" />)
    ...
    fireEvent.click(screen.getByRole("button", { name: "Save" }))
    await waitFor(() => expect(updateTaskSupply).toHaveBeenCalledWith("h1", "t1", 0, { name: "16×25×1 MERV 8 filter", url: null, size: "16x25x1" }))
    expect(await screen.findByText("16×25×1 MERV 8 filter")).toBeTruthy()
