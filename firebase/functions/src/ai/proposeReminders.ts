...
import { VALID_SCHEDULE_TYPES } from "../../../../shared/parse/parseCore.js"
      ...
      proposals: {
        ...
        items: {
          ...
          properties: {
            ...
            suggested_schedule_type: { type: "string", enum: VALID_SCHEDULE_TYPES.filter((s) => s !== "setup") },
 ...
 * validated proposals. Every safety rule lives here, not in the prompt.
 */
export async function proposeCore(rows: TemplateRow[], focusText: string, callTool: CallClaudeTool): Promise<ProposedRow[]> {
  ...
  const raw = await callTool({
    ...
    content: [{ type: "text", text: buildUserPrompt(rows, focusText) }],
  })
  ...
  for (const p of list) {
    ...
    if (!row) continue // hallucinated or not in this home — dropped, never trusted
    seen.add(id)

    let sched = typeof o.suggested_schedule_type === "string" && (VALID_SCHEDULE_TYPES as string[]).includes(o.suggested_schedule_type)
...
export const proposeReminders = onCall(
  ...
  async (request) => {
    ...
    if (rows.length === 0) return { ok: true as const, total_templates: 0, proposals: [] }

    const proposals = await withAiQuota(db, uid, "proposeReminders", () =>
      proposeCore(rows, focusText.trim(), makeCallClaudeTool(ANTHROPIC_API_KEY.value())),
    )
