...
## In Progress
- [ ] **Beta round 11 — the add-item flow, designed once instead of patched again** (owner decisions 2026-08-24; 6 reports, all owner-reported, all filed inside 7 minutes on the flow shipped 2026-08-22/23). All six approved Fix now, then deliberately HELD: "I don't want to keep revisiting these menus and just repeating, reporting these UI issues." Three of the six are earlier reports returning (HH-73 → HH-107, HH-63 → HH-110, HH-96 → HH-111), and in each case the earlier fix added a warning, collapsed a section, or moved a field instead of changing the outcome.
  - [ ] **HH-110 — the parent. At least five complete add-item flows, every step drawn**, judged on two things: effortless add, and the activation moment (seeing the parsed information of the *correct* item). Front-end design best practices, not a patch to the current screen. Blocked on the owner's answers to the clarifying questions.
  - [ ] **HH-111 — purchase date, retailer and price have no home in the add flow.** `IdentifyStep.tsx:1192` still says they "live on the Purchase step now"; PR #161 retired that step, so the comment is false and the fields are gone. HH-96 returning. Folds into the HH-110 design.
  - [ ] **HH-112 — the name defaults to brand + model.** Should default to something recognisable (the item type, already captured as `subType`), drop the model number, and say the name can be changed any time. Folds into HH-110.
  - [ ] **HH-109 — step 2 puts the Upload PDF tab above the search card and its own drop zone below it.** Verified against production first: this IS the shipped design, not a stale bundle. The file's own comment says search is "deliberately not the default path"; the layout says otherwise. Folds into HH-110.
  - [ ] **HH-107 — manual search still offers results it has already judged bad.** A parts list from guaranteedparts.com carrying our own "probably has no upkeep in it" warning, and a manuals.plus result titled "Manuals", both with a filled primary "Use this". Needs a **confidence gate in `rankCandidates`**, not another badge — the round-7 fix added information and the complaint returned. Pairs with the brand registry in `design/manual-sourcing-and-parse-cache.md`, which supplies the manufacturer URL for the no-match state.
  - [ ] **HH-108 — the manual preview cannot be exited.** CARVED OUT: not a flow question and shippable on its own. `sheet.tsx:76` renders the close X with no padding around a `size-4` icon — a 16x16 tap target against Apple's 44px minimum, in the SHARED component, so every sheet in the app has it. In this sheet it is the only exit (88vh, no swipe-to-dismiss, no Done in the footer). Also: the footer says "This is the current reference page" on a sheet opened to vet a search result.
