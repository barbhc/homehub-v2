...
## In Progress
- [x] **Beta round 12 — the review reaches every door** (owner decisions 2026-08-25; 7 reports on the flow deployed an hour earlier). ALL SEVEN LIVE: PR #169, deployed and verified in the production bundle.
  - [x] **HH-119 (P0) — the consolidated review reached one door of three.** `TaskReviewSheet` had three callers and one passed `focus`; the other two fell back to the pre-round-10 full review, so attaching a manual from the ITEM page had never had the approved design. The safe value is now the default.
  - [x] **HH-120 (P0) — my regression from #167.** The inline review rendered inside the pickup card, a narrow flex row, and collapsed to one word per line behind a second review. Built once, rendered in exactly one place: replacing the card in flow, a drawer out of it.
  - [x] **HH-114 — the spec suggestions were STRANDED by round 11.** They lived inside "Add more details", which stopped mounting for the appliance lane; I had checked CategoryFields and the Name field and not these. Now on the main column under the identity card. Deliberately NOT rendered as confirmed facts — see `ProductSuggestionCard`'s header for why.
  - [x] **HH-115 — order was right, weight was not.** Upload is a tall card with the only filled button; search sits below a rule in muted type with a paragraph naming how it goes wrong. No Scan button until there is something to scan.
  - [x] **HH-116 + HH-117 — one source for the leave-is-safe promise** (`lib/scanCopy.ts`), naming all three exits, on every surface showing a live scan.
  - [x] **HH-118 — the tray stands down** on a page already showing that scan, carries each item's page count, and says closing the app is safe.
  - [x] **Vocabulary** — a test now walks every source file and inspects every string form. It found TEN parse/analyse offenders; round 11's grep found three, because it matched double-quoted strings and the pill's template literal escaped it.
  - [ ] **HH-108 — the manual preview cannot be exited.** The last item open across both rounds. 16px close target in the shared sheet component; every sheet and dialog inherits it. ~45 min.

- [x] **Beta round 11 — the add-item flow, designed once instead of patched again** (FIVE OF SIX LIVE: PR #167, deployed 2026-08-24, verified in the production bundle) (owner decisions 2026-08-24; 6 reports, all owner-reported, all filed inside 7 minutes on the flow shipped 2026-08-22/23). All six approved Fix now, then deliberately HELD: "I don't want to keep revisiting these menus and just repeating, reporting these UI issues." Three of the six are earlier reports returning (HH-73 → HH-107, HH-63 → HH-110, HH-96 → HH-111), and in each case the earlier fix added a warning, collapsed a section, or moved a field instead of changing the outcome.
  - [x] **HH-110 — the parent. At least five complete add-item flows, every step drawn**, judged on two things: effortless add, and the activation moment (seeing the parsed information of the *correct* item). Front-end design best practices, not a patch to the current screen. Blocked on the owner's answers to the clarifying questions.
  - [x] **HH-111 — purchase date, retailer and price have no home in the add flow.** `IdentifyStep.tsx:1192` still says they "live on the Purchase step now"; PR #161 retired that step, so the comment is false and the fields are gone. HH-96 returning. Folds into the HH-110 design.
  - [x] **HH-112 — the name defaults to brand + model.** Should default to something recognisable (the item type, already captured as `subType`), drop the model number, and say the name can be changed any time. Folds into HH-110.
  - [x] **HH-109 — step 2 puts the Upload PDF tab above the search card and its own drop zone below it.** Verified against production first: this IS the shipped design, not a stale bundle. The file's own comment says search is "deliberately not the default path"; the layout says otherwise. Folds into HH-110.
  - [x] **HH-107 — manual search still offers results it has already judged bad.** A parts list from guaranteedparts.com carrying our own "probably has no upkeep in it" warning, and a manuals.plus result titled "Manuals", both with a filled primary "Use this". Needs a **confidence gate in `rankCandidates`**, not another badge — the round-7 fix added information and the complaint returned. Pairs with the brand registry in `design/manual-sourcing-and-parse-cache.md`, which supplies the manufacturer URL for the no-match state.
  - [x] **HH-113 — the resume gate said "You have an incomplete setup"** while holding the item's name, brand, model, step, manual state and start time — which made "Start fresh", the one irreversible button on the screen, a guess about what you were discarding. LIVE (#168): names the item, says the one thing missing, says when you started. Its pre-round-11 title was also why the owner's first screen did not look like the new flow.
  - [x] **The first screen, actually built to the approved design** (#168). PR #167 changed two strings on it and claimed it matched. Now: no "Add more details" disclosure on the appliance lane (not merely inert — not mounted), no stepper anywhere in the add flow, approved subtitle and model link. Back kept deliberately: the mockup drew no lane chooser before screen 01, the real flow has one, and it is a state rather than a route.
  - [x] **Two capabilities rescued before deleting anything** (#168) — `CategoryFields` (fuel type, filter type, installation date, service provider) rendered ONLY inside that disclosure, and the item's NAME had no editor anywhere in the app. Both now live in Details & records; the name is the first rename this app has ever had, which HH-112 had asked us to advertise.
  - [x] **OCR must not name an appliance** (#168) — it set the name from the nameplate ("Coway AP-1512HH"), and `composeItemName` keeps any name it is handed, so every photo-assisted add would have quietly undone HH-112. Now simple-lane only.
  - [ ] **HH-108 — the manual preview cannot be exited.** STILL OPEN, carved out on purpose: not a flow question and shippable on its own. `sheet.tsx:76` renders the close X with no padding around a `size-4` icon — a 16x16 tap target against Apple's 44px minimum, in the SHARED component, so every sheet in the app has it. In this sheet it is the only exit (88vh, no swipe-to-dismiss, no Done in the footer). Also: the footer says "This is the current reference page" on a sheet opened to vet a search result.
...
**Merging ships nothing.** `.github/workflows/ci.yml` is the only workflow —
there is no deploy job and no `deploy` npm script. Firebase Hosting is a manual
`npx firebase-tools deploy --only hosting --project homehub-2068d`, and the iOS
shell loads the live site, so that push is what reaches testers. Always verify by
fetching the production bundle, never by the deploy's exit code.
