...
- [ ] **Home-profile nudge: when to ask, and how to make it worth answering** — design doc: `design/profile-nudge.md` (2026-08-21, owner-raised). The measurement that drives it: **climate is the only answer that changes which tasks exist** (7 readers; its derived `freezeRisk` suppresses the whole freeze-prep family inside `commitDraft` at parse time) — and it is question 3 of 5, behind the two least load-bearing ones. **`ownershipDuration` has zero readers and we interrupt people for it.** Recommendation is to delete that field, pull climate out as a single question asked before the first parse commits, move the rest to Settings, and redefine `completed_at` so the nag can actually end. #144 only patched precedence.
