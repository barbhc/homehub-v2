...
TASK RULES (be exhaustive about genuine UPKEEP, but CURATE — a homeowner wants the handful of maintenance/care jobs they'd actually want reminders for, not a transcription of the manual. Emit a task for EVERY distinct recurring MAINTENANCE / CARE procedure the manual describes, plus every genuine setup step. Do NOT emit operating / usage / one-time-config steps — those are how you USE or set preferences on the appliance, not upkeep (see NOT A TASK below). A typical consumer appliance yields ~4-10 real upkeep tasks; if you are over ~12 for one appliance, you are extracting usage steps — cut them.):
...
- SCAN THE WHOLE MANUAL for recurring care, not just a section titled "Maintenance": care steps are often embedded in operation, feature, "for best results", and troubleshooting sections. Emit a distinct task for each recurring procedure the manual mentions, including easy-to-miss ones: filters / screens / lint traps; sensors and probes (e.g. moisture sensors); seals / gaskets / door boots; interior surfaces, drum, or tub; exterior surfaces; drains, pumps, and hoses; vents / ducts / airflow paths; descaling or self-clean cycles; periodic inspections. If the owner does it more than once over the appliance's life, it is a recurring task.
...
- priority_tier: essential = a genuine SAFETY hazard (fire, shock, gas, CO, tip-over) OR a lapse that clearly voids warranty or causes failure within a year — this is RARE, usually 0-2 per appliance; recommended = extends lifespan / preserves function; optional = cosmetic or convenience. Routine wiping, polishing, exterior cleaning, and comfort steps are NEVER essential. Self-check: if you marked more than ~2 tasks essential for one appliance, you are inflating — demote the ones that are merely "good to do" to recommended.
...
NOT A TASK — DO NOT EMIT (these are how you USE or set up the appliance, not upkeep a homeowner needs reminding of). For each candidate, ask: "Would a homeowner want a recurring REMINDER for this, or is it just operating / personalizing the appliance?" If it is operation or a preference, DROP it — do not emit it as a task at any tier:
- OPERATION / USAGE — the normal act of using the appliance: refilling a consumable you top up to run it (fill/replace water in the tank, add beans/detergent/rinse aid for a normal cycle), running a normal cycle, loading/unloading, adding ingredients, selecting a mode, "position the water tank arm", "assemble unit for use", "inspect paddle before each use". (EXCEPTION: replacing a MAINTENANCE consumable on a cadence — filters, cartridges, descaling — IS a task.)
- ONE-TIME CONFIG / PERSONALIZATION: WiFi/app pairing, "connect to WiFi", custom presets, "program custom cup volume", factory reset, setting the clock, unit/temperature preferences, "set up ...".
- UNBOXING ARTIFACTS: removing shipping film / stickers / foam / transit bolts (one-time, trivial — not upkeep). Genuine install checks (leveling, grounding, clearance) are still setup tasks; removing a sticker is not.
- Pure informational or "for best results" tips that describe no repeatable maintenance action.
When a step is borderline between usage and care, prefer DROPPING it — a shorter, trusted list beats a complete-but-noisy one.
