...
| inventory | **itemService + supplyService + useServiceProviders = ported**; storageService = STANDALONE increment (needs membership storage.rules [currently Phase-1 DENY-ALL] + async Firebase download URLs replacing getPhotoUrl's sync Supabase public URL across render paths + Storage emulator wired into test:e2e:emu + searchProductImages→callable); manualSourcesService/ocrService/planGenerationService/productLookupService = Bucket B callables; legacy inventoryService = stub | mixed |
