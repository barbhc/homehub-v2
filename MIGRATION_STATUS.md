...
| care | **weekAgenda.getWeekAgenda = ported** (single denormalized read, no joins); **taskService mark/snooze/getTaskDetail = ported** (getTaskDetail = instance denorm + template inline schedule/supplies + done-count neverCompleted, emu-verified); taskService templates CRUD (getTaskInstances/getTaskTemplates/create/update/archive/delete/tier+completion history), taskScheduleService, homeUpkeep, careNoteService, scheduleService, shoppingListService = stub | mixed |
...
| knowledge | parseManualService — trust-arc API (startParse/watchParse/parseManualAndWait/toUiStage) on Firebase = **ported**; shim `parseManual` still present for 5 callers (Phase 5); **knowledgeService FAQ subset = ported** (saveFaq/getFaqsByHome/getFaqsByItem/deleteFaq → homes/{homeId}/chatFaqs, homeId threaded, emu-verified); knowledgeService chunk/parse-correction fns (getChunksBy*, searchChunks, getKnowledgeChunksByHome, reclassify/convert/archive/updateChunk*, logParseCorrection) still on shim (unverifiable w/o seed chunks); manualDocumentService, chatService, conversationService, detectDocTypeService, diagramRenderService = stub; previewManualService + saveManualParseService = DELETE (worker modes) | mixed |
